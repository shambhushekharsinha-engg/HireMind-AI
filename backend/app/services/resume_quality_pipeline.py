...
class ResumeQualityPipeline:
    ...
    ACTION_VERBS = {
        "built", "developed", "architected", "engineered", "implemented", "managed",
        "led", "designed", "created", "increased", "decreased", "reduced", "optimized",
        "spearheaded", "transformed", "orchestrated", "automated", "streamlined",
        "expanded", "generated", "launched", "maximized", "pioneered", "scaled"
    }
    ...
    def analyze(self, raw_text: str, parsed_sections: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        metrics_found = re.findall(r"\b(?:\d+%\b|\$\d+|\d+\s*k|\d+\+|\d+\s*million|\d+\s*percent|\d+)\b", text, re.IGNORECASE)
        ...
        present_sections = 0
        if parsed_sections:
            present_sections = sum(1 for sec in self.SECTION_KEYWORDS if sec in parsed_sections and parsed_sections[sec])
        else:
