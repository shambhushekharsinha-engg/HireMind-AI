...
class RAGPromptBuilder:
    ...
    def build_prompt(question: str, context_docs: List[Any]) -> Dict[str, str]:
        context_str = "\n---\n".join([d[0].get("content", "") if isinstance(d, tuple) else d.get("content", "") for d in context_docs])
        system_prompt = (
...
class AnswerValidator:
    ...
    def validate(answer: str, question: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "validated_answer": answer if is_valid else f"Here is key career guidance regarding '{question}': Focus on measurable outcomes and technical skills match."
        }
...
class CareerCoachService:
    ...
    def _seed_knowledge_base(self):
        kb_docs = [
            {"id": "kb-1", "title": "ML Engineer Career Roadmap", "content": "To become an ML Engineer, master Python, SQL, and linear algebra first. Build proficiency in PyTorch/TensorFlow, Docker, and FastAPI for model serving. Work on end-to-end projects like real-time anomaly detection or sentiment analysis APIs."},
            {"id": "kb-2", "title": "ATS Scoring Criteria", "content": "ATS scores depend on 5 key factors: 1) Relevant technical skills match, 2) Complete section headers, 3) Standard 300-1000 word length, 4) Quantifiable metrics in bullet points, and 5) Clean formatting without tables."},
            {"id": "kb-3", "title": "Portfolio Project Recommendation", "content": "Top recommended projects for software candidates: 1) Full-Stack SaaS application with OAuth & Payments, 2) Microservice REST API with Docker & CI/CD, 3) AI-powered RAG document search interface, 4) Real-Time WebSocket dashboard."}
        ]
    ...
    def ask_coach(self, question: str) -> Dict[str, Any]:
        ...
        if "ml" in q_lower or "machine learning" in q_lower:
            ...
            raw_answer = f"Focusing on continuous skill acquisition, tailoring your resume with measurable achievements (XYZ formula), and practicing mock interview questions will significantly increase your callback rate."
