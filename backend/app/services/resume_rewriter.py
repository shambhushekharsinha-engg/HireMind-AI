...
class SelectiveResumeRewriter:
    ...
    def rewrite_bullets(cls, bullet_points: List[str]) -> Dict[str, Any]:
        ...
        for bullet in bullet_points:
            is_weak = any(re.search(pat, bullet, re.IGNORECASE) for pat in cls.WEAK_PATTERNS) or not re.search(
                r"\d", bullet
            )
            ...
            if is_weak:
                rewritten_count += 1
                cleaned = re.sub(
                    r"\b(worked on|responsible for|helped with|assisted in|handled)\b",
                    "",
                    bullet,
                    flags=re.IGNORECASE,
                ).strip()
                ...
                option_executive = (
                    f"Spearheaded strategic initiative to deploy {cleaned}, driving key operational goals."
                )
                option_metrics = (
                    f"Architected and deployed {cleaned}, improving throughput by 35% and reducing latency by 40ms."
                )
