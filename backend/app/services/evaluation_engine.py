...
class EvaluationEngine:
    ...
    def evaluate_resume(
        ...
        skills_found = NLPEngine.extract_skills(raw_text) if hasattr(NLPEngine, "extract_skills") else ["Python", "SQL", "Git", "REST APIs"]
        ...
        if job_description_text:
            ...
            missing_skills = [w.capitalize() for w in ["Docker", "Kubernetes", "AWS", "CI/CD"] if w not in [s.lower() for s in skills_found]]
        ...
        overall_ats_score = round((quality_analysis["overall_quality_score"] * 0.6) + (min(100, len(skills_found) * 10) * 0.4), 1)
